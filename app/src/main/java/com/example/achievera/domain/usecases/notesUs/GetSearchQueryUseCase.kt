...
class GetSearchQueryUseCase @Inject constructor(private val notesRepository: INotesRepository) {
    operator fun invoke(query: String): Flow<PagingData<NotesDatabaseElement>> {
        Log.d("NotesListViewModel", "Заметки найдеры: ${notesRepository.getSearchQuery(query,0)}")
        return notesRepository.getSearchQuery(query,0)
    }
}